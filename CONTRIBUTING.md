...
Every version bump (a change to the `Version` field in
[DESCRIPTION](https://toscm.github.io/toscutil/DESCRIPTION)) should be
marked with a git tag and an accompanying GitHub release. This is done
manually after the version bump has been merged into `master` — there is
**no** GitHub Action that creates tags or releases automatically (the
workflows in
[.github/workflows](https://toscm.github.io/toscutil/.github/workflows)
only run R CMD check, test coverage and pkgdown).

For a version `X.Y.Z`:

1.  Create an annotated tag on the `master` commit that bumped the
    version and push it (omit the commit to tag the current `HEAD`):
...
2.  Create a GitHub release from that tag, using the version’s section
    from [NEWS.md](https://toscm.github.io/toscutil/NEWS.md) as the
    release notes:
    ...
    The newest version (highest semantic version) is automatically
    marked as “Latest”.

Tag and release names always use the `vX.Y.Z` form, matching the
